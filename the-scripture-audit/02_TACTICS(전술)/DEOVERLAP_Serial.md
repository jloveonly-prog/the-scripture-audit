<!-- doc_no: 20260829_0006 | ver: 20260909_1816 -->
...
```
✅ 직렬 배치 예 — 여호람(유다)·여호사밧 공동섭정 (왕하 8:16, 왕하 1:17):
   본문이 "여호사밧이 그때 유다 왕이었는데(Jehoshaphat being then king) 여호람이 통치하기 시작"이라고 **명시**
   → 요람(이스라엘) 5년: 여호람 공동섭정 시작 → 여호사밧 사망(요람 8년경) → 단독 통치 → 요람 12년 사망·아하지야 승계
   → "동시에"가 원문에 있는 이 구간만 겹침으로 처리하고, 나머지는 직렬로 배치한다.

⚠️ 예외 — 총계 정형구 안의 하위 기간은 직렬로 덧붙이지 않는다 (2026-09-09 판례 교훈):
   대하 21:18-20  "발병 → 2년 후 사망 → [요약] 32세 즉위, 8년 통치하고 떠났더라"
   ❌ 8년 통치 → 2년 투병 = 10년 (32+8+2=42)   ← 구 판례의 오류. 요람 5→12년 창(8년)과 충돌, CP-SAT INFEASIBLE
   ✅ 투병 2년은 8년 **안**의 마지막 2년 → 32+8 = 40세 사망. 대하 22:2의 42는 나이가 아니라 오므리 왕조력(69−27).
   규칙: "N년 통치하고 죽었다"류 **총계 정형구**가 하위 사건(투병·전쟁·도피 등)을 감싸고 있으면,
         그 하위 기간은 총계 안에 포함된 것으로 두고, 독립 앵커(타국 동기화 등)로 총량을 반드시 검산한다.
```

**📌 CHRONICLE 전투기록**: `[A+C+G+AC]_아하지야_22세vs42세_오므리왕조력.md`
