<!-- doc_no: 20260829_0154 | ver: 20260909_1816 -->
...
| **C-09** | **다중 좌표 해석 충돌** | 동일 인물/사건에 대해 두 기록이 서로 다른 시점·시간축을 가리키는 경우 | 아하지야 22(개인 나이 축, 왕하 8:26) vs 42(오므리 왕조력 축, 대하 22:2) — 같은 즉위 순간의 두 시간축 |
...
✅ 관련 구절을 더 모으면 (열쇠 발견):
  왕하 8:26  → 22세                      ← 충돌 구절 A
  대하 22:2  → 42세                      ← 충돌 구절 B
  대하 21:5/21:20 → 여호람 32세 즉위, 8년 통치   ← 앵커(부친 축): 40세 사망 → "42=부친 나이" 봉쇄
  왕하 8:16/8:25  → 요람 5년 즉위 → 12년 승계    ← 앵커(이스라엘 축): 8년 창 확인
  왕상 16:16      → 오므리 즉위 = 아사 27년      ← 제3의 앵커 ★ (왕조 원점)
  왕상 16:29·22:41·왕하 3:1·8:25 → 아사력 38→41→58→69 ← 제3의 앵커 ★ (연쇄 동기화)

  → 69 − 27 = 42  ← 오므리 왕조력이었다!
  → 22세 = 아하지야 본인 나이 / 42 = 같은 순간의 왕조력 좌표
  → 모순 아님. 서로 다른 원점의 숫자. ✅ (구버전 "32+8+2=42 부친 나이"는 CP-SAT INFEASIBLE로 기각)
...
[COMBO-VERIFY — 이중 검증 점화 (의무 실행)]
  ...
  COMBO-E6: TYPE-A + TYPE-K  → 연대기 직렬 역산 + 생물학적 정합 검증
                               📌 전투기록: [A+C+G+AC]_아하지야_22세vs42세_오므리왕조력.md
                               여호람 32세→40세 직렬 역산(A, 요람 5→12년 창 검산) + "18세 득남·사망 시 젖먹이 후계(요아스 7세 즉위)" 부자 나이 생물학적 정합(K)
