...
classDiagram
    ...
    class Michael {
        +class: SonsOfGod_First
        +title: Archangel
        +militaryRank: ChiefPrince
        +KJV_Dan10v13: chief_prince
        +KJV_Jude1v9: archangel
    }
    class SonsOfGod_Second {
        +parent: Adam_BeforeFall
        +descent: present
        +bodyType: Physical_BodyOfDust
        +flesh: sarx
        +reproduction: true
        +sinStatus: Sinless_BeforeFruit
    }
    class Angel {
        +role: MinisteringSpirit
        +royalty: false
        +singing: false_saying_only
        +wings: false
        +reproduction: false
        NOTE: No_promotion_to_Archangel
    }
    class ThroneCreatures {
        <<abstract>>
    }
    class Cherubim {
        +wings: 4
        +faces: 4_man_lion_ox_eagle
        +position: BelowThrone
        +role: Guard
    }
    class Seraphim {
        +wings: 6
        +faces: 1
        +position: AboveThrone
        +role: Worship
        +attribute: Burning
    }
    class Ophanim {
        +structure: WheelInWheel
        +eyes: fullRim
        +position: BesideCherubim
        +role: Mobility
    }
    class FallenSons {
        +status: Fallen
    ...
    class Devils {
        ...
        +bodyStatus: Lost_Destroyed
        +seeking: HumanBody
        +location: Earth_Wandering
    }
    class ImprisonedSpirits {
        +location: Tartarus
        +bondage: EternalChains
        +releaseTime: WhiteThrone
    }
    class EuphratesAngels {
        +location: RiverEuphrates
        +cavalry: 200million
        +mission: Kill_OneThird
        +releaseTime: SpecificTime
    }
    class Abaddon {
        +NotSatan: true
        +location: BottomlessPit
        +title: KingOfAbyss
