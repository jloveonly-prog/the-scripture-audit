...
```mermaid
sequenceDiagram
    participant B as Me (Believer)
    participant S as Mary & Saints (Terminals)
    participant G as God (Server)
    
    Note over B, G: 🔴 The contradictory 'Smartphone Server' communication structure of Catholicism
    B->>G: 1. My prayer already reaches Almighty God
    G-->>S: 2. The server unnecessarily tosses the prayer to the terminals (Mary & Saints)(?)
    S->>G: 3. The terminals ask the server (God) for that prayer again(?)
    
    Note over B, G: 🟢 The gospel communication structure spoken by the Bible (Heb 4:16)
    B->>G: Direct connection to the throne of grace completed through Jesus' blood!
```
