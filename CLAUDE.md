...
## 이 디렉토리 (01.TheScriptureAudit — KO, 원본, 접미사 없음)

- QVCAP 감사 엔진(한글판). `the-scripture-audit`, `the-catholic-audit`, `the-sermon-audit` 세 하위 유닛을 포함.
- **이 저장소가 원본(source of truth)**입니다. 여기서 작성/검증된 문서를 영문으로 번역해서 `01.TheScriptureAudit-en`(EN, 하이픈 접미사)로 이관합니다.
- (참고) `01.TheScriptureAudit-ko`는 2026-09-05 재편 시점까지의 기록으로 동결된 별도의 과거 이력 보관소입니다 — 더 이상 활성 원본이 아니며 번역·이관 대상도 아닙니다. 폴더 접미사는 언더스코어(`_ko`/`_en`)가 아니라 **하이픈(`-ko`/`-en`)**입니다.
